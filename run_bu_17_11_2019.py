...
Model = collections.namedtuple("Model", "rate_predictor_loss, predict_rate, outputs, predict_real, predict_fake, discrim_loss, discrim_grads_and_vars, gen_loss_GAN, gen_loss_L1, gen_grads_and_vars, train")
...
def load_examples_v2():
    ...
    with tf.name_scope("load_images"):
        ...
        # labels = tf.cast(tf.substr(paths,0,label_length),dtype='int32')
        # labels = tf.string_to_number(tf.substr(paths,0,label_length),tf.float32)
        # pdb.set_trace()
        labels = tf.string_to_number(tf.substr(paths,suffix_length,label_length),tf.float32)
...
def create_model(inputs, targets, rate_labels):
    ...
    update_losses = ema.apply([discrim_loss, gen_loss_GAN, gen_loss_L1, rate_predictor_loss])
